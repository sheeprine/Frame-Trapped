...
//
// ----------------------------------------------------------------------------------------
namespace FrameTrapped.Input.Utilities.SignatureScanner
{
    public class SigScan
    {
        ...
        private bool MaskCheckInFile(int nOffset, byte[] btPattern, string strMask, byte[] bytesInFile)
        {
            // Loop the pattern and compare to the mask and dump.
            for (int x = 0; x < btPattern.Length; x++)
            {
                ...
                // If the mask char is not a wildcard, ensure a match is made in the pattern.
                if ((strMask[x] == 'x') && (btPattern[x] != bytesInFile[nOffset + x]))
                    return false;
            }
        ...
        public int FindPatternInFile(string filename, byte[] pattern, string strMask, int nOffset)
        {
            try
            {
                if (strMask.Length != pattern.Length)
                    return -1;

                byte[] bytesInFile = File.ReadAllBytes(filename);

                for (int x = 0; x < bytesInFile.Length; x++)
                {
                    if (this.MaskCheckInFile(x, pattern, strMask, bytesInFile))
                    {
                        return (int)this.m_vAddress + (x + nOffset);
                    }
                }
